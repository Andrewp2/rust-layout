...
- Added a source-controlled observed-performance report example, serializer coverage, and an opt-in `GLASSWORKS_PERFORMANCE_REPORT_DIR` artifact writer so elapsed samples can be persisted/reported as diagnostics without becoming flaky pass/fail thresholds.
...
- Added a Foundation Checks workflow that runs native performance-budget fixtures with `GLASSWORKS_PERFORMANCE_REPORT_DIR` and uploads the observed JSON reports as a CI artifact.
...
- Added caller-visible GDS import warnings for split incoming layer mappings, so one GDS layer number fanning out into multiple Glassworks layers is reported instead of only being implied by generated-layer rows.
...
- Hardened GDS import reports so zero or negative PATH widths that are salvaged into positive Glassworks path widths now emit caller-visible normalization warnings instead of only tracing an internal parser warning.
...
- Hardened GDS export loss reporting so semantic Glassworks shape kinds that cannot round-trip through plain GDS, such as vias and measurements, now emit explicit caller-visible warnings and native status summaries.
...
- Hardened GDS import reporting for duplicate structure names, warning callers about ambiguous `STRNAME` definitions and avoiding orphan duplicate cells while importing duplicate definitions into one Glassworks cell.
...
- Hardened GDS import loss reporting so coordinates that overflow Glassworks coordinate range during DBU scaling now emit caller-visible clamped-coordinate warnings and native import status summaries instead of only internal trace logs.
...
- Hardened GDS export loss reporting so zero or negative Glassworks path widths are exported with a positive GDS PATH width and reported in both model-level export warnings and native status summaries.
