...
impl DxfExportReport {
    pub fn entity_count(&self) -> usize {
        self.polyline_count + self.text_count
    }
}
    ...
    for flattened in document.flattened_shapes() {
        ...
        match &mut shape.kind {
            ShapeKind::Rectangle(rect) => {
                write_dxf_lwpolyline(&mut text, layer_name, &rect.corners(), true, None);
            ...
            ShapeKind::Polygon(poly) => {
                ...
                write_dxf_lwpolyline(&mut text, layer_name, &points, true, None);
                report.shape_count += 1;
                report.polyline_count += 1;
            }
            ShapeKind::Path { points, width } => {
                ...
                write_dxf_lwpolyline(&mut text, layer_name, &points, false, Some(*width));
                report.shape_count += 1;
                report.polyline_count += 1;
            }
            ...
            ShapeKind::Label {
                ...
                write_dxf_text(&mut text, layer_name, *position, label, document.grid);
                report.shape_count += 1;
                report.text_count += 1;
            }
            ShapeKind::Measurement { a, b, label, .. } => {
                write_dxf_lwpolyline(
                    &mut text,
                    layer_name,
                    &[*a, *b],
                    false,
                    Some(document.grid.max(1)),
                );
                write_dxf_text(&mut text, layer_name, *b, label, document.grid);
                report.shape_count += 1;
                report.polyline_count += 1;
                report.text_count += 1;
...
pub fn import_dxf_with_report(input: &str) -> Result<DxfImportResult, DxfError> {
    ...
    while index < pairs.len() {
        ...
        if section_name.as_deref() == Some("ENTITIES") {
    ...
    while *index < pairs.len() {
        ...
        *index += 1;
        match entity_type.as_str() {
            "ENDSEC" | "EOF" => break,
            "POLYLINE" => {
                parse_old_polyline_entity(pairs, index, document, layer_map, report)?;
            }
            "LINE" | "LWPOLYLINE" | "CIRCLE" | "ARC" | "TEXT" | "MTEXT" => {
                ...
                match entity_type.as_str() {
                    "LINE" => {
                        parse_line_entity(&pairs[start..*index], document, layer_map, report)?
                    }
                    "LWPOLYLINE" => {
                        parse_lwpolyline_entity(&pairs[start..*index], document, layer_map, report)?
                    }
                    "CIRCLE" => {
                        parse_circle_entity(&pairs[start..*index], document, layer_map, report)?
                    }
                    "ARC" => parse_arc_entity(&pairs[start..*index], document, layer_map, report)?,
                    "TEXT" | "MTEXT" => {
                        parse_text_entity(&pairs[start..*index], document, layer_map, report)?
                    }
                    _ => unreachable!(),
                }
            }
...
fn parse_lwpolyline_entity(
    ...
    insert_polyline_geometry(document, layer, points, closed, width, "LWPOLYLINE", report);
    Ok(())
}
...
fn parse_old_polyline_entity(
    ...
    insert_polyline_geometry(document, layer, points, closed, width, "POLYLINE", report);
    Ok(())
}
